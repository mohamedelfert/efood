...
<body class="footer-offset">
    ...
    <script>
        ...
        messaging.onMessage(function (payload) {
            ...
            if (payload.data && payload.data.order_id) {
                if (payload.data.type == "order_request" || payload.data.type == "order_canceled") {
                    // Let our notification system handle it
                    if (window.NotificationSystem) {
                        window.NotificationSystem.checkNow();
                    }

                    if (payload.data.type == "order_canceled") {
                        toastr.error(payload.data.body || 'Order has been canceled', payload.data.title || 'Order Canceled');
                        playAudio();
                    } else {
                        // Show modal only for new orders
                        $('#popup-modal').appendTo("body").modal('show');
                    }
                }
            }
    ...
    <script src="{{asset('public/assets/admin/js/notification-system.js')}}"></script>

...
<script>
    ...
    (function() {
        ...
        // Notification counts
        let lastMessageCount = parseInt($('.btn-status-c1').first().text()) || 0;
        let lastOrderCount = parseInt($('.btn-status-c1').last().text()) || 0;
        
        // Create audio element for notification sound
        const notificationSound = new Audio('{{ asset("public/assets/admin/sound/notification.mp3") }}');
        notificationSound.volume = CONFIG.soundVolume;
        
        // Fallback: Create notification sound using Web Audio API if file doesn't exist
        ...
        function playNotificationSound() {
            ...
            notificationSound.play().catch(error => {
                // If audio file doesn't exist or fails, use beep sound
        ...
        function showDesktopNotification(title, body, icon) {
            if (!CONFIG.showDesktopNotification) return;
            
            if ('Notification' in window && Notification.permission === 'granted') {
                const notification = new Notification(title, {
                    body: body,
                    icon: icon || '{{ asset("public/assets/admin/img/logo.png") }}',
                    badge: '{{ asset("public/assets/admin/img/logo.png") }}',
        ...
        function checkNewNotifications() {
            $.ajax({
                url: '{{ route("admin.get-notification-count") }}',
                type: 'GET',
                success: function(response) {
                    const newMessageCount = response.message_count || 0;
                    const newOrderCount = response.order_count || 0;
                    ...
                    if (newMessageCount > lastMessageCount) {
                        ...
                        showDesktopNotification(
                            ...
                            `{{ translate("You have") }} ${diff} {{ translate("new message(s)") }}`,
                            '{{ asset("public/assets/admin/img/icons/message.png") }}'
                        );
                        
                        // Show toast notification
                        if (typeof toastr !== 'undefined') {
                            toastr.info(`{{ translate("You have") }} ${diff} {{ translate("new message(s)") }}`, '{{ translate("New Message") }}', {
                                closeButton: true,
                                progressBar: true,
                                timeOut: 5000
                            });
                    ...
                    if (newOrderCount > lastOrderCount) {
                        const diff = newOrderCount - lastOrderCount;
                        playNotificationSound();
                        showDesktopNotification(
                            '{{ translate("New Order") }}',
                            `{{ translate("You have") }} ${diff} {{ translate("new order(s)") }}`,
                            '{{ asset("public/assets/admin/img/icons/order.png") }}'
                        );
                        
                        // Show toast notification
                        if (typeof toastr !== 'undefined') {
                            toastr.success(`{{ translate("You have") }} ${diff} {{ translate("new order(s)") }}`, '{{ translate("New Order") }}', {
                                closeButton: true,
                                progressBar: true,
                                timeOut: 5000
                            });
                    ...
                    $('.btn-status-c1').first().text(newMessageCount);
                    $('.btn-status-c1').last().text(newOrderCount);
                    
                    // Show/hide badges based on count
                    if (newMessageCount === 0) {
                        $('.btn-status-c1').first().hide();
                    } else {
                        $('.btn-status-c1').first().show();
                    }
                    
                    if (newOrderCount === 0) {
                        $('.btn-status-c1').last().hide();
                    } else {
                        $('.btn-status-c1').last().show();
                    }
                },
        ...
        $(document).ready(function() {
            ...
            setInterval(checkNewNotifications, CONFIG.checkInterval);
        ...
        function addSoundToggleButton() {
            const toggleButton = `
                <li class="nav-item d-none d-sm-inline-block">
                    <button id="soundToggle" class="btn btn-icon btn-ghost-secondary rounded-circle" 
                            title="{{ translate('Toggle notification sound') }}">
                        <i class="tio-notifications-on-outlined"></i>
                    </button>
                </li>
            `;
            
            // Insert button before the account menu
            $('.navbar-nav.align-items-center.flex-row li').first().before(toggleButton);
            ...
            $('#soundToggle').on('click', function() {
                ...
                if (CONFIG.soundEnabled) {
                    ...
                    $(this).removeClass('btn-ghost-danger').addClass('btn-ghost-secondary');
                    
                    if (typeof toastr !== 'undefined') {
                        toastr.success('{{ translate("Notification sound enabled") }}');
                    }
                    ...
                    $(this).removeClass('btn-ghost-secondary').addClass('btn-ghost-danger');
                    
                    if (typeof toastr !== 'undefined') {
                        toastr.info('{{ translate("Notification sound disabled") }}');
                    }
                }
            ...
            if (savedPreference !== null) {
                CONFIG.soundEnabled = savedPreference === 'true';
                if (!CONFIG.soundEnabled) {
                    $('#soundToggle i').removeClass('tio-notifications-on-outlined').addClass('tio-notifications-off-outlined');
                    $('#soundToggle').removeClass('btn-ghost-secondary').addClass('btn-ghost-danger');
                }
            }
        }

                    ...
                    <li class="nav-item d-none d-sm-inline-block">
                        <div class="hs-unfold">
                            <a class="js-hs-unfold-invoker btn btn-icon btn-ghost-secondary rounded-circle"
                                ...
                                <span
                                    class="btn-status btn-status-c1">{{ \App\Model\Order::where(['checked' => 0, 'branch_id' => auth_branch_id()])->count() }}</span>
                            </a>
                        </div>
                    </li>
                        ...
                        <div class="hs-unfold">
                            <a class="js-hs-unfold-invoker navbar-dropdown-account-wrapper media gap-2"
                                ...
                                <div class="media-body d-flex align-items-end flex-column">
                                    <span class="card-title h5">{{auth_branch()->name}}</span>
                                    <span class="card-text fz-12 font-weight-bold">{{translate('Branch Admin')}}</span>
                                </div>
                                <div class="avatar avatar-sm avatar-circle">
                                    <img class="avatar-img"
                                        onerror="this.src='{{asset('public/assets/admin/img/160x160/img1.jpg')}}'"
                                        src="{{auth_branch()->image_full_path}}" alt="Image Description">
                                ...
                                <div class="dropdown-item-text">
                                    <div class="media align-items-center">
                                        <div class="avatar avatar-sm avatar-circle mr-2">
                                            <img class="avatar-img"
                                                onerror="this.src='{{asset('public/assets/admin/img/160x160/img1.jpg')}}'"
                                                src="{{auth_branch()->image_full_path}}"
                                                alt="{{ translate('branch image') }}">
                                        </div>
                                        <div class="media-body">
                                            <span class="card-title h5">{{auth_branch()->name}}</span>
                                            <span class="card-text">{{auth_branch()->email}}</span>
                                        </div>
                                    </div>
                                </div>

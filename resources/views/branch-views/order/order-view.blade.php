                    ...
                    <div class="row justify-content-md-end mb-3">
                        <div class="col-md-9 col-lg-8">
                            <dl class="row">
                                ...
                                <dd class="col-6 text-dark text-right">
                                    {{ \App\CentralLogics\Helpers::set_symbol($add_ons_cost) }}</dd>
                                ...
                                <dd class="col-6 text-dark text-right">
                                    {{ \App\CentralLogics\Helpers::set_symbol($sub_total = $sub_total + $total_tax + $add_ons_cost - $total_dis_on_pro + $add_ons_tax_cost - $order['coupon_discount_amount'] - $order['extra_discount']) }}
                                </dd>


                                <dt class="col-6">
                                    <div class="d-flex max-w220 ml-auto">{{translate('delivery')}} {{translate('fee')}}:
                                    </div>
                                </dt>
                                <dd class="col-6 text-dark text-right">
                                ...
                                <dd class="col-6 border-top pt-2 fz-16 font-weight-bold text-dark text-right">
                                    {{ \App\CentralLogics\Helpers::set_symbol($sub_total + $del_c) }}</dd>
                                ...
                                <dd class="col-6 text-dark text-right">
                                    {{ Helpers::set_symbol($order->order_change_amount?->paid_amount) }}</dd>
        ...
        <div class="col-lg-4">
            ...
            <div class="card mb-3">
                            ...
                            <div class="dropdown-menu text-capitalize" aria-labelledby="dropdownMenuButton">
                                @if($order['payment_method'] == 'offline_payment' && $order->offline_payment?->status != 1)
                                    ...
                                    @if($order['order_type'] != 'dine_in')
                                        ...
                                        @if(!in_array($order['order_type'], ['in_car', 'in_restaurant']))
                                            <a class="dropdown-item offline-payment-order-alert"
                                ...
                                @else
                                    ...
                                    @if(in_array($order['order_type'], ['in_car', 'dine_in', 'in_restaurant']))
                                        ...
                                        <a class="dropdown-item route-alert"
                                            data-route="{{route('branch.orders.status', ['id' => $order['id'], 'order_status' => 'in_prepare'])}}"
                ...
                @if($order->customer || $order->is_guest == 1)
                    ...
                    @if(in_array($order['order_type'], ['delivery', 'self_pickup', 'take_away']) && !$order['delivery_man_id'])
                    ...
                    @if($order['order_type'] != 'pos' && $order['order_type'] != 'take_away' && ($order['order_status'] != DELIVERED && $order['order_status'] != RETURNED && $order['order_status'] != CANCELED && $order['order_status'] != FAILED && $order['order_status'] != COMPLETED))
                        ...
                        <div class="form-control justify-content-between">
                            <span class="ml-2 ml-sm-3 ">
                                ...
                                <i class="tio-edit p-2 d-none cursor-pointer" id="edit-icon" data-toggle="modal"
                ...
                <ul class="list-group">
                    @foreach(\App\Model\DeliveryMan::where(['is_active' => 1])->whereIn('branch_id', [0, auth('branch')->id()])->get() as $deliveryMan)
                        <li class="list-group-item d-flex flex-wrap align-items-center gap-3 justify-content-between">
                            ...
                            <a id="{{$deliveryMan->id}}" class="btn btn-primary btn-sm assign-deliveryman"
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <input type="text" name="address_type" class="form-control"
                            placeholder="{{translate('EX : Home')}}" value="{{ $address['address_type'] ?? '' }}"
                            required>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="floor" placeholder="{{translate('EX : 5')}}"
                            value="{{ $address['floor'] ?? '' }}">
                    </div>
                    <div class="form-group">
                        <label>{{translate('house')}}</label>
                        <input type="text" class="form-control" name="house" placeholder="{{translate('EX : 21/B')}}"
                            value="{{ $address['house'] ?? '' }}">
                    </div>
                    <div class="form-group">
                ...
                <form action="{{route('branch.orders.increase-preparation-time', ['id' => $order->id])}}" method="post">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group flex-between">
                            <div class="badge text-info shadow cursor-pointer change-food-preparation-time"
                                data-minute="10">{{ translate('10min') }}</div>
                            <div class="badge text-info shadow cursor-pointer change-food-preparation-time"
                                data-minute="20">{{ translate('20min') }}</div>
                            <div class="badge text-info shadow cursor-pointer change-food-preparation-time"
                                data-minute="30">{{ translate('30min') }}</div>
                            <div class="badge text-info shadow cursor-pointer change-food-preparation-time"
                                data-minute="40">{{ translate('40min') }}</div>
                            <div class="badge text-info shadow cursor-pointer change-food-preparation-time"
                                data-minute="50">{{ translate('50min') }}</div>
                            <div class="badge text-info shadow cursor-pointer change-food-preparation-time"
                                data-minute="60">{{ translate('60min') }}</div>
                        </div>
                    </div>
        ...
        <div class="modal-content">
            ...
            <div class="btn--container justify-content-center my-2 mx-3">
                @if($order->offline_payment?->status == 0)
                    <a type="reset" class="btn btn-secondary update-verification-status"
                        data-status="2">{{ translate('Payment_Did_Not_Received') }}</a>
                @endif
                <a type="submit" class="btn btn-primary update-verification-status"

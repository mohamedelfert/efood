...
<div class="pos-invoice-container" id="printableAreaContent">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    ...
                    @php($addOnsCostPos += ($addOnPrices[$key2] ?? 0) * ($addOnQtys[$key2] ?? 1))
                    @php($addOnsTaxCostPos += ($addOnTaxes[$key2] ?? 0) * ($addOnQtys[$key2] ?? 1))
                    @endforeach
                </td>
            ...
            @php($subTotalPos += $amount)
            @php($totalTaxPos += $detail['tax_amount'] * $detail['quantity'])
            @endif
            @endforeach
        </tbody>
    </table>
    ...
    <div class="summary-section">
        <div class="summary-row">
            ...
            <span>{{Helpers::set_symbol($subTotalPos)}}</span>
        </div>
        @if($addOnsCostPos > 0)
            <div class="summary-row">
                <span>{{translate('تكلفة الإضافات')}}:</span>
                <span>{{Helpers::set_symbol($addOnsCostPos)}}</span>
            </div>
        @endif
        ...
        <div class="summary-row">
            ...
            <span>{{Helpers::set_symbol($totalTaxPos + $addOnsTaxCostPos)}}</span>
        </div>
        <div class="summary-row">
        ...
        @php($grand_total = $subTotalPos + $addOnsCostPos + $totalTaxPos + $addOnsTaxCostPos - $order['coupon_discount_amount'] - $order['extra_discount'] + ($order['order_type'] == 'take_away' ? 0 : $order['delivery_charge']))
        ...
        @if ($order->order_partial_payments->isNotEmpty())
            @foreach($order->order_partial_payments as $partial)
                <div class="summary-row">
                    ...
                    <span>{{ Helpers::set_symbol($partial->paid_amount) }}</span>
                </div>
            @endforeach
            <div class="summary-row" style="font-weight: 700;">
                <span>{{translate('Due Amount')}}:</span>
                <span>{{ Helpers::set_symbol($order->order_partial_payments->first()?->due_amount) }}</span>
            </div>
        @endif

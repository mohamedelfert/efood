                ...
                <td class="content">
                    ...
                    @if (isset($data) && $data->image)
                        <div class="banner-container">
                            <img class="mail-img-banner"
                                src="{{ asset('storage/app/public/email_template/' . $data->image) }}"
                                onerror="this.src='{{ asset('public/assets/admin/img/blank2.png') }}'" alt="Banner">
                        </div>
                    @endif
                    ...
                    @if (isset($data) && $data->button_url)
                        <div class="action-container">
                            <a href="{{ $data->button_url }}"
                                class="submit-btn">{{ $data->button_name ?? translate('Explore_Now') }}</a>
                        </div>
                    @endif
                ...
                <td class="footer">
                    ...
                    @if (isset($social_media) && count($social_media) > 0)
                        <div class="social-links">
                            @foreach ($social_media as $social)
                                <a href="{{ $social->link }}" class="social-btn"
                                    style="{{ (isset($data[$social->name]) && $data[$social->name] == 1) ? '' : 'display:none;' }}">
                    ...
                    <div class="privacy-links">
                        ...
                        @if (isset($data['cancelation']) && $data['cancelation'] == 1)
                            <a href="{{ route('cancellation-policy') }}">{{ translate('Cancelation_Policy') }}</a>
                        @endif
                        @if (isset($data['contact']) && $data['contact'] == 1)
                            <a href="{{ route('contact-us') }}">{{ translate('About_Us')}}</a>
                        @endif
                    </div>
                    <div class="copyright">
                        {{ $copyright_text ?? '' }}

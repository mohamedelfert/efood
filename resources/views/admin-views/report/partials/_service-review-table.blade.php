...
<table class="table table-bordered table-hover table-striped">
    ...
    @foreach($reviews as $k => $review)
        <tr>
            ...
            <td>
                @if($review->service_ratings && is_array($review->service_ratings) && count($review->service_ratings) > 0)
                    <div class="d-flex flex-column gap-1">
                        @foreach($review->service_ratings as $aspect)
                            <small>
                                <strong>{{ ucfirst(str_replace('_', ' ', $aspect['aspect'])) }}:</strong> 
                                {{ $aspect['rating'] }} <i class="tio-star text-warning" style="font-size: 10px;"></i>
                            </small>
                        @endforeach
                    </div>
                @else

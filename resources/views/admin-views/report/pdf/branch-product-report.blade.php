...
<body>
    ...
    <div class="stats-container">
        <div class="stats-row">
            ...
            <div class="stat-box">
                <h3>{{ $totalQty }}</h3>
                <p>{{ translate('total_items_sold') }}</p>
            </div>
            <div class="stat-box">
                <h3>{{ \App\CentralLogics\Helpers::set_symbol($totalAmount) }}</h3>
                <p>{{ translate('total_revenue') }}</p>
            </div>
        </div>
    </div>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th style="width: 18%;">{{ translate('product_name') }}</th>
                <th style="width: 12%;">{{ translate('branch') }}</th>
                <th style="width: 18%;">{{ translate('customer') }}</th>
                <th style="width: 18%;">{{ translate('date') }}</th>
                <th style="width: 10%;" class="text-center">{{ translate('quantity') }}</th>
                <th style="width: 10%;" class="text-center">{{ translate('amount') }}</th>
            </tr>
        </thead>
        <tbody>
            ...
            <tr>
                ...
                <td>
                    @if(isset($row['branch']) && $row['branch'])
                        <span class="branch-badge">{{ $row['branch']->name }}</span>
                    @else
                        <span style="color: #999;">{{ translate('not_available') }}</span>
                    @endif
                </td>
                <td>
                    @if(isset($row['customer']) && $row['customer'])
                        <div>
                            <span class="customer-name">{{ $row['customer']->f_name }} {{ $row['customer']->l_name }}</span><br>
                            <span class="phone-number">{{ $row['customer']->phone }}</span>
                        </div>
                    @else
            ...
            @endforeach
            <tr class="total-row">
                <td colspan="6" class="text-center">{{ translate('total') }}</td>

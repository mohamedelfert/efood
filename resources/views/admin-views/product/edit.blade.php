                        ...
                        <div class="col-lg-6">
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    @if (!empty($data) && is_array($data) && array_key_exists('code', $data[0]))
                                        ...
                                        @foreach($data as $lang)
                                            @php
                                                $translate = [];
                                                if (!empty($product['translations']) && is_array($product['translations']) && count($product['translations'])) {
                                            ...
                                            <div class="row g-2">
                                                <div class="col-12">
                                                    <div class="">
                                                        ...
                                                        <input type="text" class="form-control" name="tags" placeholder="Enter tags" value="{{ implode(',', $product->tags->pluck('tag')->toArray()) }}" data-role="tagsinput">
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                            ...
                                            @if (isset($product->variations) && !empty($product->variations))
                                                @php
                                                    ...
                                                    if (is_string($product->variations)) {
                                                        ...
                                                        $variations = (json_last_error() === JSON_ERROR_NONE) ? $decoded : [];
                                                    } elseif (is_array($product->variations)) {
                                                        $variations = $product->variations;
                                                    }
                                                @endphp
                                                @foreach ($variations as $key_choice_options => $item)
                                                    @if (!isset($item["price"]))
...
@push('script_2')
    ...
    <script>
        ...
        var count = {{ isset($product->variations) && !empty($product->variations) ? count(is_string($product->variations) ? json_decode($product->variations, true) : $product->variations) : 0 }};
                    ...
                    <div class="card view_new_option mb-2">
                        ...
                        <div class="card-body">
                            <div class="row g-2">
                                ...
                                <div class="col-lg-3 col-md-6">
                                    <div class="form-group">
                                        <label class="input-label text-capitalize d-flex align-items-center"><span class="line--limit-1">{{ translate('messages.selection_type') }}</span></label>
                                        <div class="resturant-type-group border">
                                            <label class="form-check form--check mr-2 mr-md-4">
                                                <input class="form-check-input" type="radio" value="multi" name="options[${count}][type]" id="type${count}" checked onchange="show_min_max(${count})">
                                                <span class="form-check-label">{{ translate('Multiple') }}</span>
                                            </label>
                                            <label class="form-check form--check mr-2 mr-md-4">
                                                <input class="form-check-input" type="radio" value="single" name="options[${count}][type]" id="type${count}" onchange="hide_min_max(${count})">
                            ...
                            <div id="option_price_${count}">
                                <div class="border rounded p-3 pb-0 mt-3">
                                    ...
                                    <div class="row mt-3 p-3 mr-1 d-flex" id="add_new_button_${count}">
                                        <button type="button" class="btn btn-outline-primary" onclick="add_new_row_button(${count})">{{ translate('Add_New_Option') }}</button>
                                    </div>
                                </div>
                            </div>
                        </div>

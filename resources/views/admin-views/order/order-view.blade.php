                        ...
                        <div class="col-sm-6 d-flex flex-column justify-content-between">
                            <div>
                                ...
                                <h5 class="text-capitalize">
                                    <i class="tio-shop"></i> {{translate('branch')}} : <label
                                        class="badge-soft-info px-2 rounded">
                                        {{$order->branch ? $order->branch->name : translate('Branch deleted!')}}
                                    </label>
                                </h5>
                                <div class="mt-2 d-flex flex-column">
                                    <h5 class="text-capitalize">
                                        <i class="{{$orderTypeInfo['icon']}}"></i> {{$orderTypeInfo['title']}} <label
                                            class="{{$orderTypeInfo['badge_class']}} px-2 rounded">{{$orderTypeInfo['description']}}</label>
                                    </h5>
                                    @if($orderTypeInfo['show_table_details'])
                                        <div class="hs-unfold">
                                            <h5 class="text-capitalize">
                                                <i class="tio-table"></i> {{translate('table no')}} : <label
                                                    class="badge badge-secondary">{{$orderTypeInfo['table_number'] ?? translate('N/A')}}</label>
                                            </h5>
                                        </div>
                                        <div class="hs-unfold">
                                            <h5 class="text-capitalize">
                                                <i class="tio-user"></i> {{translate('number of people')}} : <label
                                                    class="badge badge-secondary">{{$orderTypeInfo['number_of_people'] ?? translate('N/A')}}</label>
                                            </h5>
                                        </div>
                                    @endif
                                    @if($orderTypeInfo['show_car_details'])
                                        <div class="hs-unfold">
                                            <h5 class="text-capitalize">
                                                <i class="tio-car"></i> {{translate('Car_Number')}} : <label
                                                    class="badge badge-secondary">{{$orderTypeInfo['car_registration'] ?? translate('N/A')}}</label>
                                            </h5>
                                        </div>
                                        <div class="hs-unfold">
                                            <h5 class="text-capitalize">
                                                <i class="tio-paint-bucket"></i> {{translate('Car_Color')}} : <label
                                                    class="badge badge-secondary">{{$orderTypeInfo['car_color'] ?? translate('N/A')}}</label>
                                            </h5>
                                        </div>
                                    @endif
                                </div>
                            ...
                            <div>
                                <h5>{{translate('Cutlery Option')}} : <span
                                        class="{{ $order['is_cutlery_required'] == 1 ? 'badge-soft-success' : 'badge-soft-danger' }}">{{$order['is_cutlery_required'] == 1 ? translate('On') : translate('Off')}}</span>
                                </h5>
                                @if($order['order_note'])
                        ...
                        <div class="col-sm-6">
                            <div class="text-sm-right">
                                ...
                                <div class="d-flex gap-3 justify-content-sm-end my-3">
                                    ...
                                    @if(in_array($order['order_type'], ['in_car', 'take_away', 'dine_in', 'in_restaurant', 'branch']) && in_array($order['order_status'], ['confirmed', 'processing', 'out_for_delivery']))
                                        @if($order['order_status'] == 'confirmed')
                                            <span
                                                class="badge-soft-info px-2 rounded text-capitalize">{{translate('order_received')}}</span>
                                        @elseif($order['order_status'] == 'processing')
                                            <span
                                                class="badge-soft-warning px-2 rounded text-capitalize">{{translate('order_processing')}}</span>
                                        @elseif($order['order_status'] == 'out_for_delivery')
                                            <span
                                                class="badge-soft-warning px-2 rounded text-capitalize">{{translate('order_ready')}}</span>
                                        @endif
                                    @else
                                        ...
                                        @elseif($order['order_status'] == 'canceled')
                                            <span
                                                class="badge-soft-danger px-2 rounded text-capitalize">{{translate('canceled')}}</span>
                                        @else
                                            <span
                                ...
                                @if($order['order_status'] == 'canceled' && $order['cancel_reason'])
                                    <div class="d-flex gap-3 justify-content-sm-end my-3">
                                        <div class="text-dark font-weight-semibold">{{translate('Cancel Reason')}} :</div>
                                        <span class="text-dark">{{$order['cancel_reason']}}</span>
                                    </div>
                                @endif
                                ...
                                @if(!in_array($order['payment_method'], ['cash_on_delivery', 'wallet_payment', 'offline_payment']))
                                    @if($order['transaction_reference'] == null && $order['order_type'] != 'pos' && $order['order_type'] != 'dine_in' && $order['order_type'] != 'in_car')
                                        <div class="d-flex gap-3 justify-content-sm-end align-items-center mb-3">
                                            {{translate('reference')}} {{translate('code')}} : <button
                                                class="btn btn-outline-primary px-3 py-1" data-toggle="modal"
                                                data-target=".bd-example-modal-sm"> {{translate('add')}} </button>
                                        </div>
                                ...
                                <div class="d-flex gap-3 justify-content-sm-end mb-3 text-capitalize">
                                    {{translate('order_type')}} : <label class="badge-soft-info px-2 rounded">
                                        {{translate($order['order_type'])}} </label>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <table
                        ...
                        <tbody>
                            ...
                            <tr>
                                                ...
                                                @else
                                                    <div class="font-size-sm text-body">
                                                        <span class="text-dark">{{translate('price')}} :
                                                            {{Helpers::set_symbol($detail['price'])}}</span>
                                                    </div>
                                                @endif
                                                <div class="d-flex gap-2">
                                                ...
                                                <span>
                                                    ...
                                                    <div class="font-size-sm text-body">
                                                        ...
                                                        <span class="font-weight-semibold">
                                                            {{$add_on_qty}} x
                                                            {{ Helpers::set_symbol($addOnPrices[$key2]) }} <br>
                                                        </span>
                                                    </div>
                                ...
                                <td class="text-right">{{Helpers::set_symbol($amount - $totDiscount + $productTax)}}
                                </td>
                            </tr>
                    ...
                    <div class="row justify-content-md-end mb-3">
                        <div class="col-md-9 col-lg-8">
                            <dl class="row">
                                ...
                                <dd class="col-6 text-dark text-right">
                                    {{ Helpers::set_symbol($totalTax + $addOnsTaxCost) }}
                                </dd>
                                <dt class="col-6">
                                    <div class="d-flex max-w220 ml-auto">
                                ...
                                <dd class="col-6 text-dark text-right">
                                    {{ Helpers::set_symbol($subTotal = $subTotal + $totalTax + $addOnsCost - $totalDisOnPro + $addOnsTaxCost - $order['coupon_discount_amount'] - $order['extra_discount']) }}
                                </dd>
                                <dt class="col-6">
                                    <div class="d-flex max-w220 ml-auto">
                                ...
                                <dd class="col-6 border-top pt-2 fz-16 font-weight-bold text-dark text-right">
                                    {{ Helpers::set_symbol($subTotal + $del_c) }}
                                </dd>
                                                                    ...
                                                                    <dt class="col-6">
                                                                        <div class="d-flex max-w220 ml-auto">
                                                                            <span> {{translate('Paid By')}}
                                                                                ({{str_replace('_', ' ', $partial->paid_with)}})</span> <span>:</span>
                                                                        </div>
                                                                    </dt>
                                                                    <dd class="col-6 text-dark text-right"> {{ Helpers::set_symbol($partial->paid_amount) }}
                                                                    </dd>
                                                                @endforeach
                                                                <?php    $due_amount = 0;
                                ...
                                <dd class="col-6 text-dark text-right">{{ Helpers::set_symbol($changeOrDueAmount) }}
                                </dd>
                                @endif
                            </dl>
                        </div>
                                        ...
                                        @else
                                            @if(in_array($order['order_type'], ['in_car', 'take_away', 'dine_in', 'in_restaurant', 'branch']))
                                                <a class="dropdown-item route-alert"
                                                    ...
                                                    data-message="{{ translate("Change status to Order Received ?") }}"
                                                    href="javascript:">{{translate('order_received')}}</a>
                                                <a class="dropdown-item route-alert"
                                                    data-route="{{route('admin.orders.status', ['id' => $order['id'], 'order_status' => 'processing'])}}"
                                                    data-message="{{ translate("Change status to Order Processing ?") }}"
                                                    href="javascript:">{{translate('order_processing')}}</a>
                                                <a class="dropdown-item route-alert"
                                                    data-route="{{route('admin.orders.status', ['id' => $order['id'], 'order_status' => 'out_for_delivery'])}}"
                                                    data-message="{{ translate("Change status to Order Ready ?") }}"
                                                    href="javascript:">{{translate('order_ready')}}</a>
                                                <a class="dropdown-item" href="javascript:" data-toggle="modal"
                                                    data-target="#cancel_order_modal">{{translate('canceled')}}</a>
                                            @else
                                                <a class="dropdown-item route-alert"
                                ...
                                <div class="d-flex justify-content-between align-items-center gap-10 form-control">
                                    ...
                                    @if($order['payment_method'] == 'offline_payment' && $order->offline_payment?->status != 1)
                                        <label class="switcher payment-status-text">
                                            <input class="switcher_input offline-payment-status-alert" type="checkbox"
                                                name="payment_status" value="1" id="payment_status_switch"
                                    ...
                                    @else
                                        <label class="switcher payment-status-text">
                                            <input class="switcher_input change-payment-status" type="checkbox"
                                                name="payment_status" value="1" data-id="{{ $order['id'] }}"
                ...
                <div class="card-body">
                    <h4 class="mb-4 d-flex gap-2">
                        ...
                        <a href="#" data-toggle="modal" data-target="#assignDeliveryMan"
                            class="text--base cursor-pointer ml-auto"> {{translate('Change')}} </a>
                    </h4>
                    <div class="media flex-wrap gap-3">
                        <a>
                            <img class="avatar avatar-lg rounded-circle" src="{{$order->delivery_man?->imageFullPath }}"
                                alt="Image">
                        </a>
                        <div class="media-body d-flex flex-column gap-1">
                            <a target="" href="#"
                                class="text-dark"><span>{{$order->delivery_man['name'] ?? ''}}</span></a>
                    ...
                    @php($current = \App\Model\DeliveryHistory::where(['deliveryman_id' => $order['delivery_man_id'], 'order_id' => $order['id']])->latest()->first())
                        @if(isset($origin))
                            <a target="_blank" class="text-dark" title="Delivery Boy Last Location" data-toggle="tooltip"
                                data-placement="top"
                        ...
                        @endif
                    @else
                    <a href="javascript:" class="text-dark last-location-view" data-toggle="tooltip"
                        data-placement="top" title="{{translate('Only available when order is out for delivery!')}}">
                        <img width="13" src="{{asset('public/assets/admin/img/icons/location.png')}}"
                            alt="">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                        ...
                        @if($order->customer)
                            <div class="media flex-wrap gap-3">
                                <a target="_blank" class="" href="{{route('admin.customer.view', [$order->customer['id']])}}">
                                    <img class="avatar avatar-lg rounded-circle" src="{{$order->customer?->imageFullPath}}"
                                        alt="Image">
                                </a>
                                <div class="media-body d-flex flex-column gap-1">
                                    <a target="_blank" href="{{route('admin.customer.view', [$order->customer['id']])}}"
                                        class="text-dark"><strong>{{$order->customer['name']}}</strong></a>
            ...
            <form action="{{route('admin.orders.update-shipping')}}" method="post">
                ...
                <div class="modal-body">
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                <label class="input-label" for="">{{ translate('contact_person_name') }} <span
...
@if($order['order_type'] != 'pos' && $order['order_type'] != 'take_away' && ($order['order_status'] != DELIVERED && $order['order_status'] != RETURNED && $order['order_status'] != CANCELED && $order['order_status'] != FAILED && $order['order_status'] != COMPLETED))
    <div class="modal fade" id="counter-change" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog modal-sm" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title custom-text-size" id="exampleModalLabel">
                        {{ translate('Need time to prepare the food') }}
                    </h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
...
<div class="modal fade" tabindex="-1" role="dialog" aria-labelledby="cancelOrderModal" id="cancel_order_modal"
    aria-hidden="true">
    <div class="modal-dialog modal-md" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title h4" id="cancelOrderModalLabel">{{translate('Cancel Order')}}</h5>
                <button type="button" class="btn btn-xs btn-icon btn-ghost-secondary" data-dismiss="modal"
                    aria-label="Close">
                    <i class="tio-clear tio-lg"></i>
                </button>
            </div>
            <form action="{{ route('admin.orders.status') }}" method="get">
                <input type="hidden" name="id" value="{{$order['id']}}">
                <input type="hidden" name="order_status" value="canceled">
                <div class="modal-body">
                    <div class="form-group">
                        <label>{{translate('Cancel Reason')}}</label>
                        <textarea class="form-control" name="cancel_reason" rows="3" required
                            placeholder="{{translate('Enter cancel reason...')}}"></textarea>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">{{translate('Close')}}</button>
                    <button type="submit" class="btn btn-primary">{{translate('Cancel Order')}}</button>
                </div>
            </form>
        </div>
    </div>

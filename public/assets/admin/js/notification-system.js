 ...
 * Only notifies for NEW orders/messages, not on every page
 */
    ...
    const NotificationChecker = {
        ...
        handleResponse(res) {
            ...
            if (!state.isInitialized) {
                ...
                console.log('📊 Initial state:', {messages: newMsgCount, orders: newOrdCount});
                return;
            }
        ...
        handleNewMessages(diff) {
            console.log('🔔 New messages:', diff);
            AudioManager.play();
            ...
            if (typeof toastr !== 'undefined') {
                toastr.info(`لديك ${diff} رسالة جديدة`, 'رسالة جديدة', {
                    closeButton: true,
                    progressBar: true,
                    timeOut: 5000,
        ...
        handleNewOrders(diff) {
            console.log('🔔 New orders:', diff);
            AudioManager.play();
            DesktopNotificationManager.show(
            ...
            if (typeof toastr !== 'undefined') {
                toastr.success(`لديك ${diff} طلب جديد`, 'طلب جديد', {
                    closeButton: true,
                    progressBar: true,
                    timeOut: 5000,

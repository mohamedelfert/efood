...
class CustomerWalletController extends Controller
{
            ...
            foreach ($gateways as $gatewayType) {
                ...
                if ($gateway) {
                    ...
                    foreach ($methods as &$method) {
                        ...
                        $method['supported_currencies'] = $gatewayType === 'stripe' ? ['EGP', 'SAR', 'USD'] : ['YER', 'SAR', 'USD'];
                        $method['min_amount'] = $gatewayType === 'stripe' ? env('STRIPE_MIN_AMOUNT', 1) : 1;
                        $method['max_amount'] = $gatewayType === 'stripe' ? env('STRIPE_MAX_AMOUNT', 100000) : 50000;
    ...
    //                 // Note: stripe_transaction_id comes later in callback
    //                 $metadata = array_merge($metadata, [
    ...
    public function topUpWallet(Request $request): JsonResponse
    {
        ...
        // CALCULATE POTENTIAL CASHBACK BEFORE PAYMENT
        $cashbackPreview = null;
        try {
            $cashbackService = app(CashbackService::class);
            $cashbackPreview = $cashbackService->calculatePotentialCashback(
                'wallet_topup',
                $amount,
                $user->branch_id ?? null
            );
            ...
            Log::error('Cashback preview failed', [
                'user_id' => $user->id,
                'error' => $e->getMessage()
            ]);
        }
        ...
        try {
            ...
            if (isset($response['status']) && $response['status']) {
                ...
                // ADD CASHBACK PREVIEW TO RESPONSE
                if ($cashbackPreview && $cashbackPreview['eligible']) {
                    $responseData['cashback_preview'] = [
                        'eligible' => true,
                        'estimated_cashback' => $cashbackPreview['amount'],
                        'cashback_type' => $cashbackPreview['cashback_type'],
                        'cashback_percentage' => $cashbackPreview['percentage'] ?? null,
                        'message' => $cashbackPreview['message']
                    ];
                }

...
class OrderController extends Controller
{
    ...
    //                     if (!$walletTransaction) {
    //                         DB::rollBack();
    //                         Log::error('Wallet transaction creation failed for Stripe/QIB partial payment', ['order_id' => $o_id]);
    //                         return response()->json([
    //                             'errors' => [['code' => 'wallet_error', 'message' => translate('Failed to create wallet transaction')]]
    //                         ], 400);
    //                     }

    //                     Log::info('WalletTransaction created successfully (partial payment)', [
    //                         'transaction_id' => $transactionId,
    //                         'user_id' => $userId,
    //                         'metadata' => $metadata,
    //                         'order_id' => $o_id
    ...
    //                     $partial->due_amount = $dueAmount;
    //                     $partial->transaction_id = $transactionId;
    //                     $partial->save();
    ...
    //                     // PROCESS ORDER CASHBACK (for wallet payments and COD)
    //                     $cashbackAmount = null;
    //                     try {
    //                         // Only process cashback for wallet payments and COD (paid immediately)
    //                         if ($customer && $request->payment_method === 'wallet_payment' && !$request->is_partial) {
    //                             $cashbackService = app(CashbackService::class);
    //                             $cashbackAmount = $cashbackService->processOrderCashback(
    //                                 $customer,
    ...
    //                                     'user_id' => $customer->id,
    //                                     'order_id' => $o_id,
    //                                     'order_amount' => $calculated_order_amount,
    ...
    //                         }
    //                     } catch (\Exception $e) {
    //                         Log::error('Order cashback processing failed', [
    //                             'order_id' => $o_id,
    ...
    //                 // BUILD METADATA WITH STRING CASTING FOR NON-PARTIAL
    //                 $metadata = [
    //                     'order_id' => $o_id,
    //                     'gateway' => $request->payment_method,
    //                     'purpose' => 'order_payment',
    ...
    //                     'transaction_id' => $transactionId,
    //                     'stripe_order_id' => $paymentResponse['order_id'] ?? null,
    //                     'user_id' => $userId,
    //                     'amount' => $calculated_order_amount + $deliveryCharge,
    //                     'gateway' => $request->payment_method,
    //                     'order_id' => $o_id,
    ...
    //         if (in_array($request->payment_method, ['wallet_payment', 'stripe', 'qib', 'offline_payment'])) {
    //             $gatewayInfo = PaymentGatewayHelper::getGatewayInfo($request->payment_method);
    ...
    public function placeOrder(Request $request): JsonResponse
    {
        ...
        try {
            ...
            if ($customer && $request->payment_method === 'wallet_payment' && !$request->is_partial) {
                ...
                // PROCESS ORDER CASHBACK FOR WALLET PAYMENT (PAID IMMEDIATELY)
                $cashbackAmount = null;
                try {
                    if ($customer) {
                        $cashbackService = app(CashbackService::class);
                        ...
                        if ($cashbackAmount) {
                            Log::info('Order cashback earned (wallet payment)', [
                                'user_id' => $customer->id,
                                'order_id' => $o_id,
                                'order_amount' => $calculated_order_amount,
                                'cashback_amount' => $cashbackAmount,
                    ...
                    Log::error('Order cashback processing failed (wallet payment)', [
                        'order_id' => $o_id,
                        'user_id' => $customer ? $customer->id : null,
                        'error' => $e->getMessage()
                    ]);
    ...
    public function handleCallback(Request $request): JsonResponse
    {
        try {
            ...
            // Update transaction with Stripe transaction ID if available
            if ($stripeTransactionId && empty($transaction->metadata['stripe_transaction_id'])) {

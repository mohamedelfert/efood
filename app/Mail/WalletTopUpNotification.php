...
class WalletTopUpNotification extends Mailable
{
    ...
    {
        $email_template = EmailTemplate::with('translations')->where('type', 'user')->where('email_type', 'wallet_topup')->first();
        ...
        return $this->subject(translate('Wallet_TopUp_Mail'))->view('email-templates.new-email-format-' . $template, ['company_name' => $company_name, 'data' => $email_template, 'title' => $title, 'body' => $body, 'footer_text' => $footer_text, 'copyright_text' => $copyright_text, 'url' => $url]);
    }
}